...
export default function ReflectBackground(props) {
    const {
        tint = "#005980",
    ...
    const tintRef = useRef([0, 0.35, 0.5])
    ...
    iterationsRef.current = Number.isFinite(iterations) ? Math.round(iterations) : 5
    animationRef.current = animation !== false
    pointerStrengthRef.current = Number.isFinite(pointerStrength) ? pointerStrength / 100 : 0.4
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            const t = animationRef.current ? (performance.now() - start) / 1000 : frozenT
            frozenT = t
            ...
            pointerActiveRef.current += (pointerActiveTargetRef.current - pointerActiveRef.current) * 0.08
            gl.uniform2f(uPointer, pointerRef.current.x, pointerRef.current.y)
            gl.uniform1f(uPointerStrength, pointerStrengthRef.current * pointerActiveRef.current)
    ...
    // التعديل السحري هنا: خلينا قراءة الماوس تكون على مستوى الشاشة كلها مش الكانفاس بس
    useEffect(() => {
