...
export default function App() {
  ...
  async function handleDetectAndReport() {
    ...
    try {
      // ── الخطوة 1: طلب GPS من المتصفح ──────────────────────────────
      if (!navigator.geolocation) {
        throw new Error("متصفحك لا يدعم خاصية GPS");
      }
      ...
      // ── الخطوة 2: جلب اسم المحافظة من الإحداثيات ──────────────────
      const geoRes  = await fetch(
        `https://nominatim.openstreetmap.org/reverse?lat=${latitude}&lon=${longitude}&format=json`
      );
      const geoData = await geoRes.json();
      const governorate = geoData.address?.state ||
                          geoData.address?.county ||
                          geoData.address?.city   || "";

      setUserInfo({ latitude, longitude, governorate });

      // ── الخطوة 3: إرسال البيانات للـ Backend ──────────────────────
      ...
      // رسائل خطأ GPS واضحة للمستخدم
      if (err.code === 1) {
        setError("رفضت السماح بالوصول للموقع. يرجى السماح للمتصفح باستخدام GPS.");
      } else if (err.code === 2) {
        setError("تعذّر تحديد موقعك. تأكد من تفعيل GPS على جهازك.");
      } else if (err.code === 3) {
